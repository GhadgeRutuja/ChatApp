...
const ProfilePage = () => {
  ...
  const handleSave = async () => {
    ...
    if (picFile) {
      ...
      const res = await axios.post('http://localhost:3001/api/upload-profile-pic', formData, {
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-[#18122B] text-white">
      <div className="bg-[#282142] p-8 rounded-xl shadow-lg w-full max-w-md">
        <div className="flex flex-col items-center gap-4">
          <img
            src={user.profilePic ? `http://localhost:3001${user.profilePic}` : assets.avatar_icon}
            alt="Profile"
